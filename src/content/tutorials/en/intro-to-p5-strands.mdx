...
flowchart TD
  subgraph Vertex_Shader ["Vertex Shader Pipeline"]
      V_Start([Start Vertex]) --> BV[beforeVertex]
      BV --> OBJ[Object Space]
      OBJ -.-> GOBJ[[getObjectInputs]]
      GOBJ -.-> WORLD[World Space]
      WORLD -.-> GWRD[[getWorldInputs]]
      GWRD -.-> CAM[Camera/View Space]
      CAM -.-> GCAM[[getCameraInputs]]
      GCAM -.-> PROJ[Projection Space]
      PROJ --> AV[afterVertex]
  end
  ...
  subgraph Fragment_Shader ["Fragment Shader Pipeline"]
      ...
      L_MAT -.-> GPI[[getPixelInputs]]
      GPI -.-> BLEND[Color Blending]
      BLEND -.-> COMB[[combineColors / getColor]]
      COMB -.-> FIN[Final Output Processing]
      FIN -.-> GFC[[getFinalColor]]
      GFC --> AF[afterFragment]
  end
  AF --> End([Screen Output])
  style GOBJ fill:#f96,stroke:#333
  style GWRD fill:#f96,stroke:#333
  style GCAM fill:#f96,stroke:#333
  style GPI fill:#9cf,stroke:#333
  style COMB fill:#9cf,stroke:#333
  style GFC fill:#9cf,stroke:#333
```