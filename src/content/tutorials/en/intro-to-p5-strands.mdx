...
function getAnimatedSpherePosition() {
  ...
  let phi = rand([id, 3.321]) * PI + millis() / 10000;
...
const intensity = max(original, 0.2) * 8;
```
The `0.2` inside of `max` acts as *a kind* of threshold. Areas which are full black in the blurred image will not be affected by bloom, as they will ultimately be multiplying by 0. The `8` multiplies the overall strength of the effect.  Feel free to adjust them to find something which looks good, or control them with mouse and time inputs!
...
function starShaderCallback() {
  ...
  function semiSphere() {
    let id = instanceID();
    let theta = rand2([id, 0.1234])  * TWO_PI + millis() / 100000;
    let phi = rand2([id, 3.321]) * PI + millis() / 50000;
