...
import DynamicToC from "@components/DynamicToC/index.tsx";
...
```js
  function fresnelCallback() {
    ...
    let normalVector = normalize(cameraInputs.normal);
    // This creates a vector pointing from the surface to the camera
    let viewVector = normalize(-cameraInputs.position);
    // ...
    cameraInputs.end();
  }
```
...
<Callout title="Tip">
Try using the `mouseX` and `mouseY` values for an interactive, color changing effect instead of the hardcoded pink.
</ Callout>
...
<EditableSketch code={`
  ...
  function starShaderCallback() {
    ...
    function semiSphere() {
      let id = instanceID();
      let theta = noise([id, 0.1234])  * TWO_PI + time / 100000;
      let phi = noise([id, 3.321]) * PI + time / 50000;
...
## Experimental strands hooks pipeline diagram


```mermaid
%%{init: {'theme': 'dark'}}%%
flowchart TD
  classDef hooklesser fill:#fcc89a,stroke:#333,color:#222;
  classDef hook fill:#f96,stroke:#333,stroke-width:2px,color:#202020;
  Start([Geometry]) --> BV

  subgraph Vertex_Shader ["Vertex Shader Pipeline"]
      BV[[beforeVertex]] --> OBJ[Object Space]
      OBJ -.-> INOBJ[[objectInputs:<br/>position, normal, texCoord, color]]
      INOBJ -.-> WORLD[World Space]
      WORLD -.-> INWLD[[worldInputs:<br/>position, normal, texCoord, color]]
      INWLD -.-> CAM[Camera/View Space]
      CAM -.-> INCAM[[cameraInputs:<br/>position, normal, texCoord, color]]
      INCAM -.-> PROJ[Projection Space]
      PROJ --> AV[[afterVertex]]
  end
  AV --> Raster[Rasterization]
  subgraph Fragment_Shader ["Fragment Shader Pipeline"]
      Raster --> BF[[beforeFragment]]
      BF --> L_MAT[Lighting & Material]
      L_MAT -.-> INPIX[[pixelInputs:<br/>normal, texCoord, ambientLight, ambientMaterial, specularMaterial, emissiveMaterial, color, shininess, metalness]]
      INPIX -.-> BLEND[Color Blending]
      BLEND -.-> COMB[[combineColors:<br/>baseColor, diffuse, ambientColor, ambient, specularColor, specular, emissive, opacity, set
 ]]
      COMB -.-> FIN[Final Output Processing]
      FIN -.-> FINCOL[[finalColor:<br/>color, set]]
      FINCOL --> AF[[afterFragment]]

  end
  AF --> End([Screen Output])
  %%For filterColor - only in filterShader
  %%subgraph TODO ["TODO"]
  %%  FILCOL[[filterColor:<br/>texCoord, canvasSize, texelSize, canvasContent, set]]
  %%  end

  click INOBJ "https://beta.p5js.org/reference/p5/objectinputs/" "objectInputs Reference"
  click INWLD "https://beta.p5js.org/reference/p5/worldinputs/" "worldInputs Reference"
  click INCAM "https://beta.p5js.org/reference/p5/camerainputs/" "cameraInputs Reference"
  click INPIX "https://beta.p5js.org/reference/p5/pixelinputs/" "pixelInputs Reference"
  click COMB "https://beta.p5js.org/reference/p5/combineColors/" "combineColors Reference"
  click FINCOL "https://beta.p5js.org/reference/p5/finalcolor/" "finalColor Reference"
  click FILCOL "https://beta.p5js.org/reference/p5/filtercolor/" "filterColor Reference"

  class INOBJ,INWLD,INCAM,INPIX,COMB,FINCOL,FILCOL hook
  class BV,AV,BF,AF hooklesser

```


#### example overriding mermaid theme variables:
```
%%{init: {'theme': 'dark', 'themeVariables': { 'primaryColor': '#ff9966'}}}%%
```